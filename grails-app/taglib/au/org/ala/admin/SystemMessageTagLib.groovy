...
class SystemMessageTagLib {
    ...
    private void showSystemMessage(out, attrs) {
        ...
        if (message?.text) {
            out << "<div class='pt-3 m-3 system-message alert alert-${message.severity}'>${message.text}${attrs.showTimestamp?.toBoolean() ? ' (' + message.timestamp + ')' : ''}</div>"
        }
    }

    private void showNonProductionEnvironmentWarning(out, attrs) {
        if (isNonProductionEnvironment() && !attrs.hideEnvWarning?.toBoolean()) {
            out << """<div class="pt-3 alert alert-warning env-message">
